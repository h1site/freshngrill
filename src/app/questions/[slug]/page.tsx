...
export async function generateStaticParams() {
  const slugs = getAllQuestionSlugs('fr');
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const question = getQuestionBySlug(slug, 'fr');

  if (!question) {
    return { title: 'Question non trouvée' };
  }

  const localized = getLocalizedQuestion(question, 'fr');
...
const categoryLabels = {
  cuisson: { label: 'Temps de cuisson', icon: Clock },
  conservation: { label: 'Conservation', icon: BookOpen },
...
export default async function QuestionPage({ params }: Props) {
  ...
  const localized = getLocalizedQuestion(question, 'fr');
  const relatedQuestions = getRelatedQuestions(slug, 'fr', 3);
  const categoryQuestions = getQuestionsByCategory(question.category, 'fr')
  ...
  const breadcrumbs = [
    ...
    { name: localized.question, url: `/questions/${slug}/` },
  ];
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#F77313]">
                      Recettes québécoises authentiques depuis 2020
                    </p>
                  </div>
                </div>
              </div>
