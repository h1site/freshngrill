    ...
    recettes: {
      ...
      featured: {
        ...
        links: [
          ...
          { name: locale === 'en' ? 'Italy' : 'Italie', href: `${basePath}?${originParam}=italie`, flag: '🇮🇹' },
          { name: locale === 'en' ? 'Mexico' : 'Mexique', href: `${basePath}?${originParam}=mexique`, flag: '🇲🇽' },
          { name: locale === 'en' ? 'United States' : 'États-Unis', href: `${basePath}?${originParam}=etats-unis`, flag: '🇺🇸' },
          { name: locale === 'en' ? 'Greece' : 'Grèce', href: `${basePath}?${originParam}=grece`, flag: '🇬🇷' },
          { name: locale === 'en' ? 'India' : 'Inde', href: `${basePath}?${originParam}=inde`, flag: '🇮🇳' },
          { name: locale === 'en' ? 'United Kingdom' : 'Royaume-Uni', href: `${basePath}?${originParam}=royaume-uni`, flag: '🇬🇧' },
          { name: locale === 'en' ? 'Germany' : 'Allemagne', href: `${basePath}?${originParam}=allemagne`, flag: '🇩🇪' },
        ],
      },
    },
  };
          ...
          <nav className="hidden lg:flex items-center gap-0 ml-2 xl:ml-2">
            {navigation.map((item) => (
              ...
              >
                <Link
                  href={item.href}
                  className={`relative flex items-center gap-0.5 px-1 xl:px-1.5 2xl:px-4 py-2 text-[14px] xl:text-[14px] 2xl:text-sm font-medium text-white/80 hover:text-white transition-colors uppercase tracking-tight xl:tracking-tight 2xl:tracking-wide group ${
