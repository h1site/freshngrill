-- Schema pour les Posts/Articles de Blog
...
CREATE INDEX IF NOT EXISTS idx_posts_title_gin ON posts USING GIN (to_tsvector('french', title));
CREATE INDEX IF NOT EXISTS idx_posts_content_gin ON posts USING GIN (to_tsvector('french', content));
...
-- Storage bucket pour les images des posts
INSERT INTO storage.buckets (id, name, public)
VALUES ('post-images', 'post-images', true)
ON CONFLICT (id) DO NOTHING;

-- Politiques storage pour post-images
CREATE POLICY "Post images publiques en lecture" ON storage.objects
  FOR SELECT
  USING (bucket_id = 'post-images');

CREATE POLICY "Upload post images pour service role" ON storage.objects
  FOR INSERT
  WITH CHECK (bucket_id = 'post-images');

CREATE POLICY "Update post images pour service role" ON storage.objects
  FOR UPDATE
  USING (bucket_id = 'post-images');

CREATE POLICY "Delete post images pour service role" ON storage.objects
  FOR DELETE
  USING (bucket_id = 'post-images');
